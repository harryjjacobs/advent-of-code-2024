...
var cache = new Dictionary<string, Int64>();
...
foreach (var line in lines)
{
    ...
    foreach (var key in line)
    {
        ...
        cost += paths.Select(PathToDirections).Select(directions => FindShortestPath(directions, 25)).Min();
        prevKey = key;
    }
    result += cost * Int64.Parse(line[..^1]); // https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/member-access-operators#range-operator-
}
Console.WriteLine(result);

Int64 FindShortestPath(IEnumerable<char> directions, Int64 endLevel, Int64 level = 0)
{
    var cacheKey = String.Join("", directions) + level.ToString();
    Int64 cached;
    ...
    foreach (var key in directions)
    {
        ...
        cost += newDirections.Select(directions => FindShortestPath(directions, endLevel, level + 1)).Min();
        prevKey = key;
    }

    cache.TryAdd(cacheKey, cost);

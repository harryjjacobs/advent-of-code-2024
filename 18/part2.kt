...
fun createGrid(size: Point): Array<Array<Cell>> {
...
fun parseInput(file: String): List<Point> {
    ...
    File(file).useLines { lines -> lineList.addAll(lines.map { line -> Point(line) }) }
    return lineList
}
...
fun main(args: Array<String>) {
    val size = Point(71, 71) // example is 7x7 
    val start = Point(0, 0)
    val end = Point(size.x - 1, size.y - 1)
    val grid = createGrid(size)

    val bytes = parseInput("input")

    run breaking@ {
        bytes.forEach { 
            grid[it.y][it.x] = Cell.CORRUPTED

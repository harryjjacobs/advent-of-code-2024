class Part1 {
    ...
    static Point[] cheatMoves(Character[][] grid, Point point) {
        def points = [
            new Point(x: point.x + 1, y: point.y + 1),
            new Point(x: point.x + 1, y: point.y - 1),
            new Point(x: point.x - 1, y: point.y + 1),
            new Point(x: point.x - 1, y: point.y - 1),
            new Point(x: point.x + 2, y: point.y),
            new Point(x: point.x - 2, y: point.y),
            new Point(x: point.x, y: point.y + 2),
            new Point(x: point.x, y: point.y - 2),
        ]
    ...
    static void main(String... args) {
        ...
        path.each { point -> 
            cheatMoves(grid, point).each { move ->
                lengthDiff.add(costs[move] - costs[point] - 2)
            }
        }
